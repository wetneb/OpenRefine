...
public interface GridState {
     ...
     * 
     * @todo users of this method might actually be required to iterate up to the end
     * of the iterator to avoid resource leaks with some implementations. This should
     * be clarified by the interface. Consider exposing a closeable iterable instead.
     */
     ...
     */
    public <T extends Serializable> PartialAggregation<T> aggregateRecordsApprox(RecordAggregator<T> aggregator, T initialState, long maxRecords);
     ...
     * The limitReached flag indicates whether the aggregation stopped because
     * the limit was reached or the underlying collection was exhausted.
     */
